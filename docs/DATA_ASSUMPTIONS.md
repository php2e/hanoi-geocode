...
- Canonical local inputs are `data/hanoi_bound_2026.geojson`, `data/hanoi_wards_2026.geojson`, and a curated ranked word source such as `data/final/all_ranked_words_36.csv`.
...
- For the current 3m grid, the active word source must provide at least 2988 unique normalized slugs. In `all_ranked_words_36.csv`, the top 3071 rows are the first top-N slice that reaches that count.
